"""
GPT-specific implementation for running experiments on SimpleQA-Verified.

This is a dataset-specific variant of `main_exp/simpleQA-verified/models/gpt_client.py`:
- loads `codelion/SimpleQA-Verified` (split: `train`)
- writes outputs using a distinct filename prefix so it doesn't overwrite SimpleQA results.
"""
...
class GPTClientSimpleQAVerified:
    """
    Runner for GPT experiments using OpenAI's Batch API (SimpleQA-Verified).
    """
    ...
    def create_batch_input_file(
        ...
        input_file = (
            f"main_exp/simpleQA-verified/outputs/batch_files/"
            f"batch_input_{self.DATASET_SLUG}_{exp_type}_{model_config['short_name']}_{start_idx}-{end_idx}.jsonl"
        )
        ...
        with open(input_file, "w") as f:
            for idx in tqdm(range(start_idx, end_idx), desc=f"Creating batch input [{start_idx}-{end_idx}]"):
                ...
                batch_request = {
                    ...
                    "method": "POST",
                    "url": "/v1/responses",
                    "body": {"model": model_config["full_name"], "input": []},
                }
    ...
    def download_batch_results(
        self, batch, dataset, start_idx, end_idx, exp_type, reward_correct, reward_abstain, reward_incorrect, model_config
    ):
        ...
        output_file = (
            "main_exp/simpleQA-verified/outputs/batch_files/"
            f"batch_output_{self.DATASET_SLUG}_{exp_type}_{model_config['short_name']}_{start_idx}-{end_idx}_{timestamp}.jsonl"
        )
    ...
    def run_single_batch(
        ...
        input_file = self.create_batch_input_file(
            dataset,
            start_idx,
            end_idx,
            exp_type,
    ...
    def run_multi_batch_experiment(
        self,
        dataset,
        exp_type,
        reward_correct,
        ...
        all_results = []
        batch_jobs = []
        tracking_file = f"main_exp/simpleQA-verified/outputs/batch_files/batch_tracking_{self.DATASET_SLUG}_{exp_type}.json"
        ...
        for i in range(num_batches):
            ...
            batch_id, batch_info = self.run_single_batch(
                dataset,
                start_idx,
                end_idx,
                exp_type,
            ...
            if batch.status == "completed":
                ...
                results_df = self.download_batch_results(
                    batch,
                    dataset,
                    start_idx,
                    end_idx,
