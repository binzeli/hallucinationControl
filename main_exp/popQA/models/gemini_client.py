"""
Gemini-specific implementation for running experiments via OpenRouter.
Uses the OpenAI-compatible client pointed at the OpenRouter base URL.
"""
...
class GeminiClient:
    ...
    def __init__(self, experiments, model_name="gemini-3.1-flash-lite"):
        ...
        print(f"✅ GeminiClient initialised — model: {self.MODEL_CONFIGS[model_name]['full_name']}")
    ...
    def run_multi_batch_experiment(self, dataset, exp_type, reward_correct, reward_abstain,
        ...
        all_results = []
        tracking_file = (
            f"main_exp/popQA/outputs/gemini_batch_files/batch_tracking_{exp_type}.json"
        )
        ...
        if all_results:
            ...
            final_file = (
                f"main_exp/popQA/outputs/{model_config['short_name']}_results/"
                f"popqa_{exp_type}_results_{reward_str}_{timestamp}.csv"
            )
    ...
    def run_experiment(self, dataset, exp_type, reward_correct, reward_abstain, reward_incorrect):
        """
        Main entry point for Gemini experiments.
