...
def create_batch_input_file(dataset, start_idx, end_idx):
    ...
    input_file = f"main_exp/popQA/ablation_study/remove_confidence/outputs/batch_files/batch_input_B_{model_short}_{start_idx}-{end_idx}.jsonl"
    ...
    with open(input_file, 'w') as f:
        for idx in tqdm(range(start_idx, end_idx), desc=f"Creating batch input [{start_idx}-{end_idx}]"):
            ...
            prompt = (
                "You will get +1 point if your answer is correct, +0.4 if you answer 'I don't know', and -1 if incorrect. \n\n"
                "If you truly don't know, first say 'I don't know'. If you choose to answer, then provide your answer for it\n"
                "Next, if you said 'I don't know', then in the same response, give your best possible guess for the answer.\n\n"
...
def download_batch_results(batch, dataset, start_idx, end_idx):
    ...
    output_file = f"main_exp/popQA/ablation_study/remove_confidence/outputs/batch_files/batch_output_B_{start_idx}-{end_idx}_{timestamp}.jsonl"
    ...
    with open(output_file, 'r') as f:
        for line in f:
            ...
            if result["response"]["status_code"] == 200:
                ...
                ans, best = extract_exp2_fields(out)
                ...
                score = 0.4 if idk_flag else (1 if correct else -1)
...
def run_multi_batch_experiment(dataset, batch_size=BATCH_SIZE):
    ...
    all_results = []
    batch_jobs = []
    tracking_file = f"main_exp/popQA/ablation_study/remove_confidence/outputs/batch_files/batch_tracking_B.json"
    ...
    if all_results:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        final_file = f"main_exp/popQA/ablation_study/remove_confidence/outputs/popqa_B_batch_results_+1_-1_+0.4_{timestamp}.csv"
