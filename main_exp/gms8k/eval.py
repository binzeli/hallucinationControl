...
def parse_filename(filename):
    ...
    Example: main_exp/outputs/gms8k/gpt5_results/gms8k_scheme_b_norm_results_+1_+0_+0.4_20260308_221816.csv
    ...
    # Extract scheme name (between gms8k_ and _results)
    scheme_match = re.search(r'gms8k_(.+?)_results', basename)
...
def get_all_result_files(results_dir="main_exp/outputs/gms8k/gpt5_results"):
    """
    Find all result CSV files in the specified directory.
    Returns a dictionary mapping scheme names to their file paths.
    """
    
    pattern = os.path.join(results_dir, "gms8k_*_results_*.csv")
    files = glob.glob(pattern)
...
def main():
    ...
    output_dir = f"main_exp/gms8k/outputs/{model_name}_results/plot_idk_confidence_separate"
